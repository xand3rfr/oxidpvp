// Tank Duel, 1v1. Drive your tank, aim the turret, and fire shells that bounce off walls
// (twice). One hit and you're scrap, and your own shells can hit you after a bounce, so bank
// shots carefully. After every kill both tanks reset. First to 5 wins.
// Built on the Arena netcode: each peer drives its own tank locally and streams its position;
// the host owns the shells, hits and score.
(() => {
  const W = 1280, H = 720;
  const R = 19, SPEED = 230, DASH_SPEED = 620, DASH_TIME = 0.16, DASH_CD = 2.5;
  const FIRE_CD = 0.42, B_SPEED = 520, B_LIFE = 5, B_R = 5, DMG = 100, RESPAWN = 1.8, WIN = 5, BOUNCES = 2, MAX_SHELLS = 5;
  ...
  // Point-symmetric maze-ish layout so neither side has an advantage.
  const half = [
    { x: 220, y: 0, w: 30, h: 220 },
    { x: 220, y: 380, w: 30, h: 200 },
    { x: 400, y: 150, w: 160, h: 30 },
    { x: 400, y: 150, w: 30, h: 170 },
    { x: 420, y: 520, w: 200, h: 30 },
    { x: W / 2 - 15, y: 0, w: 30, h: 110 },
  ];
  const WALLS = [
    ...half,
    ...half.map((w) => ({ x: W - w.x - w.w, y: H - w.y - w.h, w: w.w, h: w.h })),
    { x: W / 2 - 60, y: H / 2 - 60, w: 120, h: 120 },
  ];
  ...
  GameUtil.touchControls({
    ...
    buttons: [{ side: "right", label: "Boost", onDown: () => { dashQueued = true; } }],
  });
  ...
  const newState = () => ({ p: [mkP(0), mkP(1)], b: [], sc: [0, 0], win: -1, roundT: 0 });
  ...
  function spawnBullet(o, x, y, a) {
    if (S.roundT > 0 || S.b.filter((b) => b.o === o).length >= MAX_SHELLS) return;
    // Don't let a shell spawn inside a wall when the barrel is pressed against one.
    if (inWall(x, y)) { x = S.p[o].x; y = S.p[o].y; }
    S.b.push({ x, y, vx: Math.cos(a) * B_SPEED, vy: Math.sin(a) * B_SPEED, life: B_LIFE, o, n: 0 });
  }
  ...
  function hostTick(dt) {
    ...
    for (let s = 0; s < steps; s++) {
      for (const b of S.b) {
        ...
        // Move one axis at a time so we know which way to bounce.
        b.x += b.vx * h;
        if (inWall(b.x, b.y)) { b.x -= b.vx * h; b.vx = -b.vx; b.n++; emit({ k: "w", x: Math.round(b.x), y: Math.round(b.y), o: b.o }); }
        b.y += b.vy * h;
        if (inWall(b.x, b.y)) { b.y -= b.vy * h; b.vy = -b.vy; b.n++; emit({ k: "w", x: Math.round(b.x), y: Math.round(b.y), o: b.o }); }
        if (b.n > BOUNCES) { b.dead = true; continue; }
        if (S.roundT > 0) continue;
        for (let v = 0; v < 2; v++) {
          const t = S.p[v];
          if (!t.alive || (v === b.o && b.n === 0)) continue; // your own shell only hurts after a bounce
          if (Math.hypot(t.x - b.x, t.y - b.y) >= R + B_R) continue;
          b.dead = true;
          t.hp = 0; t.alive = false;
          const scorer = v === b.o ? 1 - v : b.o;
          S.sc[scorer]++;
          emit({ k: "k", x: Math.round(t.x), y: Math.round(t.y), v });
          if (S.sc[scorer] >= WIN) { S.win = scorer; S.b.length = 0; }
          else S.roundT = RESPAWN;
          break;
        }
      }
    }
    ...
    if (S.roundT > 0 && S.win < 0) {
      S.roundT -= dt;
      if (S.roundT <= 0) {
        // New round: both tanks back to their corners, shells cleared.
        S.roundT = 0;
        S.b.length = 0;
        S.p.forEach((p, i) => Object.assign(p, { x: START[i][0], y: START[i][1], a: i ? Math.PI : 0, hp: 100, alive: true, seq: p.seq + 1 }));
  ...
  function drawPlayer(x, y, a, i, hp, isMe) {
    ...
    ctx.beginPath(); ctx.ellipse(x + 3, y + 7, R + 2, R * 0.85, 0, 0, Math.PI * 2); ctx.fill();
    ctx.save();
    ctx.translate(x, y); ctx.rotate(a);
    // treads
    ctx.fillStyle = "#1f2127";
    ctx.fillRect(-R, -R, R * 2, 8); ctx.fillRect(-R, R - 8, R * 2, 8);
    ctx.fillStyle = "rgba(255,255,255,0.12)";
    for (let k = -R + 3; k < R; k += 7) { ctx.fillRect(k, -R + 1, 3, 6); ctx.fillRect(k, R - 7, 3, 6); }
    // hull
    ctx.fillStyle = c;
    ctx.shadowColor = c; ctx.shadowBlur = 16;
    ctx.beginPath(); ctx.roundRect(-R + 3, -R + 6, R * 2 - 6, R * 2 - 12, 5); ctx.fill();
    ctx.shadowBlur = 0;
    // barrel + turret
    ctx.fillStyle = "#e5e7eb";
    ctx.fillRect(0, -4, R + 12, 8);
    ...
    if (isMe) {
      ...
      if (me.dashCd > 0) {
        ...
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.arc(x, y, R + 9, -Math.PI / 2, -Math.PI / 2 + (1 - me.dashCd / DASH_CD) * Math.PI * 2);
        ctx.stroke();
      }
    }
    void hp;
  ...
  function render(dt) {
    ...
    if (!mine.alive && winner() < 0) {
      ...
      ctx.fillText("Destroyed! Next round…", W / 2, 60);
    }
  }
  ...
  function updateHud() {
    ...
    if (w >= 0) {
      ...
      if (link && !spec) { GameUtil.sfx(won ? "win" : "lose"); GameUtil.record("tanks", won ? "win" : "loss"); }
  ...
  Lobby.mount({
    game: "tanks",
    title: "Tank Duel",
    subtitle: "Shells bounce off walls, one hit wins the round, and your own shots can get you. First to 5.",
    onStart(l) {
      ...
      if (!l.spectator) GameUtil.toast("Shells bounce twice. Watch your own!");
